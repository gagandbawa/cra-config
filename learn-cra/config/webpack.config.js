...
const getLoader = require('../templates/loaders/index');
...
module.exports = function (webpackEnv, options) {
	...
	const appPages = isEnvDevelopment ? paths.appPages : [options.page];
	const htmlPlugins = [];
	appPages.forEach((appPage) => {
		const commonOptions = {
			...
			page: appPage.name,
			loaders: getLoader(appPage.name),
		};
		htmlPlugins.push(
	...
	return {
		...
		entry: isEnvProduction ? options.page.appIndexJs : entries,
		output: {
			// The build folder.
		...
		plugins: [
			...
			new ManifestPlugin({
				fileName: isEnvProduction
					? `${options.page.name}/manifest.json`
					: 'manifest.json',
				publicPath: paths.publicUrlOrPath,
